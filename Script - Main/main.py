...
while True:
    try:
        ...
        # 2. SAFETY LOGIC (Control the LEDs)
        ...
        # 3. PREPARE JSON DATA (Added LEDs here!)
        payload = {
            "ultrasonic_dist": dist_cm,
            "water_level": water_val,
            "rain_value": rain_val,
            "led_red": red_led.value(),      # Reads 1 or 0
            "led_green": green_led.value()   # Reads 1 or 0
        }
        
        # 4. SEND TO BRIDGE
        msg_str = ujson.dumps(payload)
        client.publish(MQTT_TOPIC, msg_str)
        print(f"Sent: {status_print} | Red: {payload['led_red']} | Green: {payload['led_green']}")
